...
class GoogleCloudStorageCustomListOperator(BaseOperator):
    """
    ...
    **Example**:
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(GoogleCloudStorageCustomListOperator, self).__init__(*args, **kwargs)
        self.bucket = bucket
        self.prefix = prefix
    ...
    def execute(self, context):
        ...
        # prefix_array = self.prefix.split('/')
        # prefix_array.insert(len(prefix_array)-1,timestamp)
        # prefix_final = '/'.join(prefix_array)

        prefix_final = self.prefix.format(timestamp)
        ...
        self.log.info('Getting list of the files. Bucket: %s; Delimiter: %s; Prefix: %s',
                      self.bucket, self.delimiter, prefix_final)

        ret_list = list(hook.list(bucket=self.bucket,
                         prefix=prefix_final,
                         delimiter=self.delimiter))

        return ret_list if not self.max_results else ret_list[:self.max_results]
